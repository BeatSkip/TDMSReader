	...
	public class Reader
	{
		...
		public IList<Metadata> ReadMetadata(Segment segment)
		{
			...
			for (var x = 0; x < objectCount; x++)
			{
				...
				if (propertyCount > 0)
				{
					System.Diagnostics.Debug.Write($"## propertycount is {propertyCount} - {segment.Identifier} - path: ");
					bool firsd = true;
					foreach (var item in metadata.Path)
					{
						if (firsd)
							firsd = false;
						else
					...
					System.Diagnostics.Debug.WriteLine($" - containsraw: {segment.TableOfContents.HasRawData.ToString()} - containsmetadata: {segment.TableOfContents.HasMetaData.ToString()} - newobjects: {segment.TableOfContents.ContainsNewObjects.ToString()}");
				...
				for (var y = 0; y < propertyCount; y++)
				{
					...
					System.Diagnostics.Debug.WriteLine($"\t - title:{key}");
				}
